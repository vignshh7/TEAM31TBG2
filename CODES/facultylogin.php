...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        $stmt = $conn->prepare("SELECT user_id, password FROM users WHERE username = ? AND user_type = 'faculty'");
        ...
        if ($result->num_rows === 1) {
            ...
            $error = "No faculty found with that username.";
        }
        $stmt->close();
    }
}
...
<head>
    ...
    <title>Faculty Login - DRS Institute of Technology</title>
    <style>
        * {
            margin: 0;
            padding: 0;
